...
import Metatron.Core.Math.Primitive.aMatrix;
...
////[MIN]->[a>b=b]|[a<b=a]
	...
	public static boolean isEqual(Number a, Number b, float tollerance) {
		...
		if (isDecimal(a))
			return aMaths.isEqual(a.floatValue(), b.floatValue(), tollerance);

		return false;
	}

	public static boolean isEqual(Number a, Number b) {
		return isEqual(a, b, aMaths.FLOAT_ROUNDING_ERROR);
	}
	...
	public static Number toPrecisionOf(Number val, Number of) {
		...
		return aMaths.round(val.floatValue(), dec);
