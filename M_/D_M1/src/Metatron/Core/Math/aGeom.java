...
public abstract class aGeom {
	...
	public static aVector getLineNormalPositive(aVector from, aVector to) {
		...
		aVector rN = mid.sub(dir.crs(D).nor().scl(aMaths.PI));// rotation formula
		return rN;
	}
	...
	public static aVector getNormalPositiveX(aVector... poly) {
		aVector dir = poly[0];
		aVector lP = centroid().cpy().add((dir.cpy().nor().scl(aMaths.PI2)));
		return lP;
	}
	...
	public static float angle(aVector from, aVector to) {
		...
		if (denom < aMaths.FLOAT_ROUNDING_ERROR)
			return 0f;

		float dot = aMaths.clamp(from.cpy().dot(to.cpy()).floatValue() / denom, -1, 1);
		return aMaths.acos(dot) * aMaths.radDeg;
		...
		if (type.equals(aVector.class)) {
			return new Comparator<aVector>() {
				...
				public int compare(aVector p0, aVector p1) {
					...
					if (aMaths.isEqual(ds0, ds1, aMaths.FLOAT_ROUNDING_ERROR * 2f)) {
						ds1 += (aMaths.FLOAT_ROUNDING_ERROR * 8f);
						ds0 -= (aMaths.FLOAT_ROUNDING_ERROR);
					}
	...
	public static aVector[] genPoly(aTransform t, int n) {
		...
		float angle = aMaths.PI2 / n;

		for (int i = 0; i < n; i++) {

			aVector Dir = dir.cpy().rotate(up, (angle * i) * aMaths.radDeg);
	...
	public static aVector[] genPoly(aVector pos, aVector dir, aVector scl, int n) {
		...
		for (int i = 0; i < n; i++) {
			...
			aVector Dir = dir.cpy().rotate(new aVector(0, 0, 1), (angle * i) * aMaths.radDeg);
	...
	public static int estimateSidesRequired(float pixelSize, float radiusX, float radiusY) {
		float circumference = (float) (aMaths.PI2 * Math.sqrt((radiusX * radiusX + radiusY * radiusY) / 2f));
		...
		return aMaths.clamp(sides, minimumSides, maximumSides);
	}
