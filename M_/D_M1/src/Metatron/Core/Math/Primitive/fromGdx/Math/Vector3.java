...
public class Vector3 implements Serializable, Vector<Vector3> {
	...
	public Vector3 setFromSpherical (float azimuthalAngle, float polarAngle) {
		float cosPolar = aMaths.cos(polarAngle);
		float sinPolar = aMaths.sin(polarAngle);

		float cosAzim = aMaths.cos(azimuthalAngle);
		float sinAzim = aMaths.sin(azimuthalAngle);
	...
	public Vector3 setToRandomDirection () {
		float u = aMaths.random();
		float v = aMaths.random();

		float theta = aMaths.PI2 * u; // azimuthal angle
	...
	public boolean isOnLine (Vector3 other) {
		return len2(y * other.z - z * other.y, z * other.x - x * other.z,
			x * other.y - y * other.x) <= aMaths.FLOAT_ROUNDING_ERROR;
	}
	...
	public boolean epsilonEquals (float x, float y, float z) {
		return epsilonEquals(x, y, z, aMaths.FLOAT_ROUNDING_ERROR);
	}
