...
package Metatron.Core.Math.Primitive.fromGdx.Math;

import java.io.Serializable;

import Metatron.Core.Math.aMaths;
import Metatron.Core.Math.Primitive.fromGdx.Utils.NumberUtils;
...
public class Quaternion implements Serializable {
	...
	public Quaternion setEulerAngles (float yaw, float pitch, float roll) {
		return setEulerAnglesRad(yaw * aMaths.degreesToRadians, pitch * aMaths.degreesToRadians,
			roll * aMaths.degreesToRadians);
	}
	...
	public float getRollRad () {
		...
		return pole == 0 ? aMaths.atan2(2f * (w * z + y * x), 1f - 2f * (x * x + z * z))
			: (float)pole * 2f * aMaths.atan2(y, w);
	}

	/** Get the roll euler angle in degrees, which is the rotation around the z axis. Requires that this quaternion is normalized.
	 * @return the rotation around the z axis in degrees (between -180 and +180) */
	public float getRoll () {
		return getRollRad() * aMaths.radiansToDegrees;
	}
	...
	public float getPitchRad () {
		final int pole = getGimbalPole();
		return pole == 0 ? (float)Math.asin(aMaths.clamp(2f * (w * x - z * y), -1f, 1f)) : (float)pole * aMaths.PI * 0.5f;
	}

	/** Get the pitch euler angle in degrees, which is the rotation around the x axis. Requires that this quaternion is normalized.
	 * @return the rotation around the x axis in degrees (between -90 and +90) */
	public float getPitch () {
		return getPitchRad() * aMaths.radiansToDegrees;
	}

	/** Get the yaw euler angle in radians, which is the rotation around the y axis. Requires that this quaternion is normalized.
	 * @return the rotation around the y axis in radians (between -PI and +PI) */
	public float getYawRad () {
		return getGimbalPole() == 0 ? aMaths.atan2(2f * (y * w + x * z), 1f - 2f * (y * y + x * x)) : 0f;
	}

	/** Get the yaw euler angle in degrees, which is the rotation around the y axis. Requires that this quaternion is normalized.
	 * @return the rotation around the y axis in degrees (between -180 and +180) */
	public float getYaw () {
		return getYawRad() * aMaths.radiansToDegrees;
	}
	...
	public Quaternion nor () {
		float len = len2();
		if (len != 0.f && !aMaths.isEqual(len, 1f)) {
...
// method
	...
	public boolean isIdentity () {
		return aMaths.isZero(x) && aMaths.isZero(y) && aMaths.isZero(z) && aMaths.isEqual(w, 1f);
	}

	/** @return If this quaternion is an identity Quaternion */
	public boolean isIdentity (final float tolerance) {
		return aMaths.isZero(x, tolerance) && aMaths.isZero(y, tolerance) && aMaths.isZero(z, tolerance)
			&& aMaths.isEqual(w, 1f, tolerance);
	}
	...
	public Quaternion setFromAxis (final float x, final float y, final float z, final float degrees) {
		return setFromAxisRad(x, y, z, degrees * aMaths.degreesToRadians);
	}
	...
	public Quaternion setFromCross (final Vector3 v1, final Vector3 v2) {
		final float dot = aMaths.clamp(v1.dot(v2), -1f, 1f);
	...
	public Quaternion setFromCross (final float x1, final float y1, final float z1, final float x2, final float y2,
		final float z2) {
		final float dot = aMaths.clamp(Vector3.dot(x1, y1, z1, x2, y2, z2), -1f, 1f);
		final float angle = (float)Math.acos(dot);
...
// value
	...
	public float getAxisAngle (Vector3 axis) {
		return getAxisAngleRad(axis) * aMaths.radiansToDegrees;
	}
	...
	public float getAxisAngleRad (Vector3 axis) {
		...
		if (s < aMaths.FLOAT_ROUNDING_ERROR) { // test to avoid divide by zero, s is always positive due to sqrt
	...
	public float getAngle () {
		return getAngleRad() * aMaths.radiansToDegrees;
	}
	...
	public float getAngleAroundRad (final float axisX, final float axisY, final float axisZ) {
		...
		return aMaths.isZero(l2) ? 0f
			: (float)(2.0 * Math.acos(aMaths.clamp((float)((d < 0 ? -this.w : this.w) / Math.sqrt(l2)), -1f, 1f)));
	}
	...
	public float getAngleAround (final float axisX, final float axisY, final float axisZ) {
		return getAngleAroundRad(axisX, axisY, axisZ) * aMaths.radiansToDegrees;
	}
