...
import Metatron.Core.Math.Primitive.fromGdx.GdxRuntimeException;
...
public class Vector2 implements Serializable, Vector<Vector2> {
	...
	@Deprecated
	public float angle () {
		float angle = (float)Math.atan2(y, x) * aMaths.radiansToDegrees;
	...
	public float angle (Vector2 reference) {
		return (float)Math.atan2(crs(reference), dot(reference)) * aMaths.radiansToDegrees;
	}
	...
	public float angleDeg (Vector2 reference) {
		float angle = (float)Math.atan2(reference.crs(this), reference.dot(this)) * aMaths.radiansToDegrees;
		if (angle < 0) angle += 360;
		return angle;
	}
	...
	public Vector2 setAngle (float degrees) {
		return setAngleRad(degrees * aMaths.degreesToRadians);
	}
	...
	public Vector2 rotate (float degrees) {
		return rotateRad(degrees * aMaths.degreesToRadians);
	}
	...
	public Vector2 setToRandomDirection () {
		float theta = aMaths.random(0f, aMaths.PI2);
		return this.set(aMaths.cos(theta), aMaths.sin(theta));
	}
	...
	public boolean epsilonEquals (final Vector2 other) {
		return epsilonEquals(other, aMaths.FLOAT_ROUNDING_ERROR);
	}
	...
	public boolean epsilonEquals (float x, float y) {
		return epsilonEquals(x, y, aMaths.FLOAT_ROUNDING_ERROR);
	}
	...
	public boolean isOnLine (Vector2 other) {
		return aMaths.isZero(x * other.y - y * other.x);
	}

	@Override
	public boolean isOnLine (Vector2 other, float epsilon) {
		return aMaths.isZero(x * other.y - y * other.x, epsilon);
	}
	...
	public boolean isPerpendicular (Vector2 vector) {
		return aMaths.isZero(dot(vector));
	}

	@Override
	public boolean isPerpendicular (Vector2 vector, float epsilon) {
		return aMaths.isZero(dot(vector), epsilon);
	}
