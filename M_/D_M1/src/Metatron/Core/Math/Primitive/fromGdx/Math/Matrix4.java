...
public class Matrix4 implements Serializable {
	...
	public float getScaleX () {
		return (aMaths.isZero(val[M01]) && aMaths.isZero(val[M02])) ? Math.abs(val[M00])
			: (float)Math.sqrt(getScaleXSquared());
	}

	/** @return the scale factor on the Y axis (non-negative) */
	public float getScaleY () {
		return (aMaths.isZero(val[M10]) && aMaths.isZero(val[M12])) ? Math.abs(val[M11])
			: (float)Math.sqrt(getScaleYSquared());
	}

	/** @return the scale factor on the X axis (non-negative) */
	public float getScaleZ () {
		return (aMaths.isZero(val[M20]) && aMaths.isZero(val[M21])) ? Math.abs(val[M22])
	...
	*/
	...
	public boolean hasRotationOrScaling () {
		return !(aMaths.isEqual(val[M00], 1) && aMaths.isEqual(val[M11], 1) && aMaths.isEqual(val[M22], 1)
			&& aMaths.isZero(val[M01]) && aMaths.isZero(val[M02]) && aMaths.isZero(val[M10]) && aMaths.isZero(val[M12])
			&& aMaths.isZero(val[M20]) && aMaths.isZero(val[M21]));
	}
}