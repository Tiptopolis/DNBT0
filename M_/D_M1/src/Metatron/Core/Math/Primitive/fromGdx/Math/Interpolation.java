...
public abstract class Interpolation {
	...
	static public final Interpolation pow2InInverse = new Interpolation() {
		public float apply (float a) {
			if (a < aMaths.FLOAT_ROUNDING_ERROR) return 0;
	...
	static public class Elastic extends Interpolation {
		...
		public Elastic (float value, float power, int bounces, float scale) {
			...
			this.power = power;
			this.scale = scale;
			this.bounces = bounces * aMaths.PI * (bounces % 2 == 0 ? 1 : -1);
		}

		public float apply (float a) {
			if (a <= 0.5f) {
				a *= 2;
				return (float)Math.pow(value, power * (a - 1)) * aMaths.sin(a * bounces) * scale / 2;
			}
			a = 1 - a;
			a *= 2;
			return 1 - (float)Math.pow(value, power * (a - 1)) * aMaths.sin((a) * bounces) * scale / 2;
		}
	}
	...
	static public class ElasticIn extends Elastic {
		...
		public float apply (float a) {
			if (a >= 0.99) return 1;
			return (float)Math.pow(value, power * (a - 1)) * aMaths.sin(a * bounces) * scale;
		}
	}
	...
	static public class ElasticOut extends Elastic {
		...
		public float apply (float a) {
			if (a == 0) return 0;
			a = 1 - a;
			return (1 - (float)Math.pow(value, power * (a - 1)) * aMaths.sin(a * bounces) * scale);
		}
	}
